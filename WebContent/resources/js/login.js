		...
		$.ajax({
			...
			success : function(data) {
				...
				if (data == "correct") {
					...
					alert("completed.");
					window.location.href="/profile_mvc2";
				} else {
...
function logout() {
	if (confirm('Confirm logout?')) {
		// document.cookie = 'online' + '=;expires=Thu, 01 Jan 1970 00:00:01
		// GMT;';
		setCookie("online", "", -1);
	}
}
	...
	if (requiredReg) {
		...
		if (saveUser) {
			if (data_required[1].value != data_required[2].value) {
				...
				return false;
				/*
				 * var data = { 'num' : parseInt(cnt) + 1, 'username' :
				 * data_required[0].value, 'passwd' : data_required[1].value,
				 * 'fullname' : data_required[3].value, 'birthdate' :
				 * data_required[4].value, 'height' : data_required[5].value,
				 * 'weight' : data_required[6].value, 'blood_type' :
				 * data_required[7].value, 'age' : data_required[8].value,
				 * 'hobby' : data_required[9].value, 'education' :
				 * data_required[10].value, 'faculty' : data_required[11].value,
				 * 'major' : data_required[12].value, 'phone' :
				 * data_required[13].value, 'email' : data_required[14].value,
				 * 'detail_edu' : '', 'detail_fav' : '', 'detail_littleskill' : '' }; //
				 * localStorage.setItem('user-' + (parseInt(cnt) + 1),
				 * JSON.stringify(data)); // localStorage.setItem('countUser',
				 * (parseInt(cnt) + 1)); alert('register completed.');
